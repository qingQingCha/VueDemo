  ...
  computed: {
    ...
    filterRatings () {
      ...
      return shopRatings.filter((shopRating) => {
        ...
        /**
         * 涉及条件1：onlyShowText，是否只看有内容的评价
         * 1. 只看有内容的评价：onlyShowText:true && shopRating.text
         * 2. 没有内容的评价也看：onlyShowText:false
         * 情况：!onlyShowText || shopRating.text
         *
         * 涉及条件2：selectType，查看评价类型
         * 1. 评价类型：0-满意，1-不满意，2-全部
         * 2. 获取评价变量shopRating.rateType：0-满意，1-不满意
         * 情况：selectType == 2 || selectType == rateType
         */
        return (!onlyShowText || text.length > 0) && (selectType === 2 || selectType === rateType)
      })
    },
    positiveSize () {
      const {shopRatings} = this
      const positiveRatings = shopRatings.filter((shopRating) => {
        return shopRating.rateType === 0
      })
